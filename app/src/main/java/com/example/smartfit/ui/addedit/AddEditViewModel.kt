...
class AddEditViewModel(
    ...
    private val TAG = "AddEditViewModel" // <--- Log Tag
    ...
    init {
        if (logId != null && logId != -1) {
            Log.d(TAG, "Editing existing log ID: $logId") // <--- Log
            ...
            Log.d(TAG, "Creating new log entry") // <--- Log
        }
    }
    ...
    // --- SAVE LOGIC ---
    fun saveLog(): Boolean {
        Log.d(TAG, "Attempting to save log...") // <--- Log
        ...
        if (!isValid) {
            Log.w(TAG, "Save Failed: Validation error on inputs") // <--- Log
        ...
        viewModelScope.launch {
            ...
            Log.i(TAG, "Saving Log Success: ${state.name} ($finalType)") // <--- Log
    ...
    fun deleteLog() {
        if (logId != null && currentLog != null) {
            Log.i(TAG, "Deleting log ID: $logId") // <--- Log
            viewModelScope.launch { activityRepository.deleteLog(currentLog!!) }
        }
    }

    private fun validateInput(cat: String, wType: String, name: String, value: String, sets: String, reps: String): Boolean {
        if (name.isBlank()) return false
        if (cat == "Workout" && wType == "Strength") {
            val isWeightValid = value.isBlank() || value.toDoubleOrNull() != null
            val areSetsRepsValid = sets.isNotBlank() && sets.toIntOrNull() != null && reps.isNotBlank() && reps.toIntOrNull() != null
